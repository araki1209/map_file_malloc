...
void gmain() {
    ...
    //ファイルを開く
    const char* filname = "assets\\map.txt";
    FILE* fp;
    fopen_s(&fp,filname,"rb");
    //ファイルサイズを取得
    fseek(fp, 0, SEEK_END);
    int filSize = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    //メモリの動的確保
    char* map = (char*)malloc(filSize);
    //ファイルからメモリへ読み込む
    fread(map, sizeof(char), filSize, fp);
    //行数列数を数える
    int rows = 0;
    int cols = 0;
    int cnt = 0;
    print((let)"filSize="+filSize);
    ...
    while (notQuit) {
        ...
        int starCol = worldX / size;
        int endCol = starCol + dispCols;
        for(int c=starCol;c<endCol;c++){
